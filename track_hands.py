...
class handDetector():
    def __init__(self, image_mode=False, max_num_hands=3, min_detection_confidence=0.5, min_tracking_confidence=0.5):
        ...
        self.hands = self.mphands.Hands(self.image_mode, self.max_num_hands, self.min_detection_confidence,
...
def main():
    ...
    while True:
        ...
        cv2.putText(img, 'Client FPS:' + str(int(fps)), (10, 70), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2,
                    color=(255, 0, 0))
        cv2.imshow('img', img)
        cv2.waitKey(1)
